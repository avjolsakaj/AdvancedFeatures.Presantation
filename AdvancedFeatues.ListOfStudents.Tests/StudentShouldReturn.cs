...
public class StudentShouldReturn
{
    ...
    public void ListOfStudents_NotExcellent ()
    {
        ...
        Assert.That(notExcellent.Count(), Is.EqualTo(5));
    }

    [Test]
    public void ListOfStudents_FalseFailed ()
    {
        var failed = _students.Where(x => !x.HavePassed());

        Assert.That(failed.Any(), Is.True);

        Assert.IsFalse(failed.Count() > 2);
    }
}